...
if rank == 0:
	...
	print("-----------------------------------------------------------------------------------------------------------")
	print("******************************* Probleme resolu : Elasticite lineaire **************************************")
	print("-----------------------------------------------------------------------------------------------------------")
...
#++++++++++++++++++++++++ Global finite element model +++++++++++++++++++++++++++++++++++++++++#
bbord_glob = [];
(Gm, Gmf,  Gmd, Gmim,  Gnbd, Gmp) = plasticity.FE_Global_Model('Global.msh');
ci = 0
...
if rank == 0:
	RHS_value = '[1900., 1900., 1900.]'
if rank != 0:
	RHS_value_i = '[1900., 1900., 1900.]'
	RHS_value_m = '[1900., 1900., 1900.]'
...
if rank == 0:
	...
	(Gmd, Gmf) = cube.load_volumic_data_Global(Gmf, Gmim, Gmd, RHS_value);
...
if rank != 0:
	...
	for i in range(0, size):
		...
		(Fm[i], Fmf[i], Fmd[i], Fmim[i], Fnbd[i], bbord_fin[i], Fmp[i]) = plasticity.FE_Local_Model(
			'Fin_' + str(ind[i]) + '.msh', 1, ind[i], interface_id)
		(Fmd[i], Fmf[i]) = cube.load_volumic_data_local(Fm[i], Fmf[i], Fmim[i], Fmd[i], RHS_value_i, RHS_value_m);
				...
				for i in range(0, size):
					(uF[i], LFonG[Gbound[i]]) = \
						Resolution_gl.Local_resolution(Fmd[i], Interpo[i], uG[0:Gnbd], Gbound[i], new_region_100_fin[i],
																							uFd[i],C[i], F_index[i], FM[i], Fmim[i])
		...
		while converged[0] == 0:
			...
			if rank != 0:
				...
				for i in range(0, size):
					for i in range(0, size):
						(uF[i], LFonG[Gbound[i]]) = \
							Resolution_gl.Local_resolution(Fmd[i], Interpo[i], uG[0:Gnbd], Gbound[i],
							                               new_region_100_fin[i], uFd[i], C[i], F_index[i], FM[i], Fmim[i])
					rG_loc[Gbound[i]] += LFonG[Gbound[i]]
				loc_iter += 1
				rG_loc[Gnbd + rank - 1] = loc_iter
				#win_rG_s[rank - 1].Lock(0, lock_type=MPI.LOCK_EXCLUSIVE)
