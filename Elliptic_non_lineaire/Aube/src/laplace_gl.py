...
import os
import sys
def FE_Model_Global(mesh, Dim, NLC):
	...
	NL_coef = Dm.eval(NLC)
	md.add_initialized_fem_data('NL_c', Dm, NL_coef)
	md.add_linear_term(mim, "Grad_u.Grad_Test_u", -1, True, True)
	# Preparation of coupling
	nbd = mf.nbdof()
#	Fnbd = Fmf.nbdof()
	return(m, mf,  md,  mim,  nbd, Dm)

def FE_Model_local(mesh, Dim, NLC):
	# Creation of a simple cartesian mesh
	m = gf.Mesh('import', 'gmsh', mesh)
	# Create a MeshFem of for field of dimension 1
	mf = gf.MeshFem(m, 1)
	...
	NL_coef = Dm.eval(NLC)
	md.add_initialized_fem_data('NL_c', Dm, NL_coef)
	md.add_nonlinear_term(mim, "Grad_u.Grad_Test_u + NL_c*u*u*u*Test_u", -1, True, True);
