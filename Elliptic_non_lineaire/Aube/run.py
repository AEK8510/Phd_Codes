...
if rank == 0:
	if rank == 0:
		...
		print("-----------------------------------------------------------------------------------------------------------")
		print('******************************* Problème resolu : Thermique non linèaire **********************************')
		print("-----------------------------------------------------------------------------------------------------------")
...
if Dim == 3:
	eps = 1.e-6
(Gm, Gmf, Gmd, Gmim, Gnbd, GDm) = laplace_gl.FE_Model_Global('Global.msh', Dim, NLC)
...
if rank != 0 :
	...
	(Fm, Fmf, Fmd, Fmim, Fnbd, FDm) = laplace_gl.FE_Model_local('Fin'+str(rank)+'.msh', Dim, NLC)
		...
		while(converged[0] == 0):
			...
			if rank == 0:
				...
				uG_inter, uG, rG_j, rG_j_1, omega_new = \
					Resolution_gl.Global_resolution(solveur, rG_j_1, rG_j, ci, omega_old, omega_new, pG, pG_index, rG, Gintdofs, Gmd, Gmf, loc_iter, Dim, NLC)
			...
			win_conv.Fence()
			if rank == 0:
				norm_rG, rG = Resolution_gl.Residu_computation(Gmd, Gmim, Gnbd, Gintdofs, nprocs, rG_win_s, a, Dim, NLC, GDm, Gmd)
...
#---------------------------------- Asynchronous version without wait -------------------------------------------------#
	...
	with Timer() as t:
		while(converged[0] == 0):
			...
			if rank == 0:
				norm_new, rG = Resolution_gl.Residu_computation(Gmd, Gmim, Gnbd, Gintdofs, nprocs, rG_win_s, a, Dim, NLC, GDm, Gmd)
...
#	uC, Cmf = Post_processing.Complementary_solution('Complement.msh', uG, Gmf, Dim)
