...
def FE_Local_Model(mesh,BORD, j, interface_id, NLC):
	# Creation of a simple cartesian mesh
	m = gf.Mesh('import', 'gmsh', mesh);
	# Create a MeshFem  for field of dimension 1
	mf = gf.MeshFem(m, 1);
	...
	#print("------------", m.regions())
	md.add_linear_term(mim, 'Grad_u.Grad_Test_u ', region=m.regions()[0])
	kappa1 = mp.eval(matrix_value)
	md.add_initialized_fem_data('coef', mp, NL_Coef)
	md.add_nonlinear_term(mim, 'Grad_u.Grad_Test_u + coef*u*u*u*Test_u', region=m.regions()[1])
