...
def Local_resolution(Fmd, Interpo, uG, Gbound, new_region_100_fin, uFd, C, F_index, FM):
	...
	Fmd.solve('alpha_min', 0.8, 'lsearch', 'simplest', 'max iter', 100,  'max res', 1e-12, 'lsolver', 'superlu');
	#Fmd.solve();
	uF = Fmd.variable('u')
...
def Aitken_Asynchrone(P, ind_svd, svd_trunc):
	...
	sin_val_sum = 0
	n_svd = 0
	while (abs((frob_norm) - np.sqrt(sin_val_sum)) > 1e-10 and n_svd < svd_trunc - 1):
